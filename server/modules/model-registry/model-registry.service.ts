...
import { buildCostLedger, MIN_LEDGER_SAMPLES } from '@/modules/model-registry/cost-ledger.service.js';
import type { LLMProvider } from '@/shared/types.js';

/**
 * Providers the registry scores. Inlined when the registry was lifted out of
 * the retired Agent Swarm module, which used to own this list.
 */
const REGISTRY_PROVIDERS: LLMProvider[] = [
  'claude',
  'codex',
  'cursor',
  'opencode',
...
export type ModelRegistryPrefs = {
...
function loadSnapshot(): BenchmarkFamily[] {
  ...
  const candidatePaths = [
    ...
    path.resolve(__dirname, "../../../server/modules/model-registry/model-benchmarks.snapshot.json"),
    path.resolve(process.cwd(), "server/modules/model-registry/model-benchmarks.snapshot.json"),
    path.resolve(process.cwd(), "dist-server/server/modules/model-registry/model-benchmarks.snapshot.json"),
  ];
...
export async function refreshModelRegistry(options: { providers?: string[] } = {}): Promise<{
  ...
  ensureSchema();
  const providers = options.providers ?? REGISTRY_PROVIDERS;
  const errors: string[] = [];
  let refreshed = 0;
...
export function getStaffingPrefs(): ModelRegistryPrefs {
  ensureSchema();
  const row = getConnection()
    .prepare(
      `SELECT allowed_providers, default_orchestrator_provider, default_orchestrator_model
       FROM model_registry_prefs WHERE id = 1`,
    )
    .get() as {
...
export function setStaffingPrefs(patch: Partial<ModelRegistryPrefs>): ModelRegistryPrefs {
  ensureSchema();
  const current = getStaffingPrefs();
  const next: ModelRegistryPrefs = {
    allowedProviders:
  ...
  getConnection()
    .prepare(
      `INSERT INTO model_registry_prefs (
