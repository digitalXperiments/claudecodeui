...
test('the entry count cap evicts the least recently used entry', async () => {
  const tempDirectory = await mkdtemp(path.join(os.tmpdir(), 'session-history-cache-lru-'));
  try {
    const paths = ['a.jsonl', 'b.jsonl', 'c.jsonl'].map((name) => path.join(tempDirectory, name));
    for (const filePath of paths) {
      await writeFile(filePath, '{"type":"user"}\n', 'utf8');
    }

    const cache = createSessionHistoryCache(256 * 1024 * 1024, 2);
    const loadsBySession = new Map<string, number>();
    const loaderFor = (sessionId: string) => async () => {
      loadsBySession.set(sessionId, (loadsBySession.get(sessionId) ?? 0) + 1);
      return historyResult(sessionId);
    ...
    await cache.getFullHistory({ sessionId: 's1', transcriptPath: paths[0], loadFull: loaderFor('s1') });
    await cache.getFullHistory({ sessionId: 's2', transcriptPath: paths[1], loadFull: loaderFor('s2') });
    // Touch s1 so s2 becomes the least recently used entry.
    await cache.getFullHistory({ sessionId: 's1', transcriptPath: paths[0], loadFull: loaderFor('s1') });
    // Inserting s3 must evict s2, not s1.
    await cache.getFullHistory({ sessionId: 's3', transcriptPath: paths[2], loadFull: loaderFor('s3') });

    await cache.getFullHistory({ sessionId: 's1', transcriptPath: paths[0], loadFull: loaderFor('s1') });
    await cache.getFullHistory({ sessionId: 's2', transcriptPath: paths[1], loadFull: loaderFor('s2') });

    assert.equal(loadsBySession.get('s1'), 1);
    assert.equal(loadsBySession.get('s2'), 2);
  } finally {
    await rm(tempDirectory, { recursive: true, force: true });
  }
});
