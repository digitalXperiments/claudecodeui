...
import { decodeOmpSessionCwdDir } from '@/modules/providers/list/omp/omp-sessions.provider.js';
...
export class OmpSessionSynchronizer implements IProviderSessionSynchronizer {
  ...
  private readonly sessionsRoot = ompSessionsRoot();

  async synchronize(since?: Date): Promise<number> {
    // Oh My Pi has no separate session metadata/title index. Re-scan transcript
    // headers on startup so existing "Untitled Oh My Pi Session" rows get their
    // first-prompt title populated after upgrading CloudCLI.
    void since;
    const files = await findFilesRecursivelyCreatedAfter(this.sessionsRoot, '.jsonl', null);
  ...
  private async processSessionFile(filePath: string): Promise<ParsedSession | null> {
    ...
    let projectPath = decodeOmpSessionCwdDir(parentDir);
      ...
      try {
        ...
        if (canonicalSessionId) {
          return {
            ...
            sessionName: normalizeSessionName(sessionName, 'Untitled Oh My Pi Session'),
            jsonlPath: filePath,
          };
        }
      } finally {
