...
export type OmpSessionTokenUsage = {
  used: number;
  total: number;
  contextUsed: number;
...
function resolveContextWindow(contextUsed: number): number {
  const configured = readNumber(process.env.OMP_CONTEXT_WINDOW || process.env.CONTEXT_WINDOW);
  if (configured > 0) return configured;
  // Oh My Pi's default model catalog is currently 200k-context. This is only a
...
export function buildOmpTokenUsageFromStats(stats: unknown): OmpSessionTokenUsage | null {
...
export function readOmpSessionTokenUsage(jsonlPath: string): OmpSessionTokenUsage {
  let fileContent = '';
  try {
