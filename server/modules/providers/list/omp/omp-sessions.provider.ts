...
/**
 * Decode Oh My Pi's cwd directory encoding: `--Users-foo-bar--` → `/Users/foo/bar`
 * (best-effort — Oh My Pi replaces `/` with `-` and wraps with `--`).
 */
export function decodeOmpSessionCwdDir(dirName: string): string | null {
...
export class OmpSessionsProvider implements IProviderSessions {
  constructor(private readonly sessionsRoot = ompSessionsRoot()) {}

  /**
   * Normalize live Oh My Pi RPC events into CloudCLI chat messages.
   * Accepts either a full RPC event object or a simplified shape from omp-cli.js.
   */
  normalizeMessage(rawMessage: unknown, sessionId: string | null): NormalizedMessage[] {
    ...
    // Pre-normalized helper shape from omp-cli.js
    if (typeof raw.kind === 'string') {
      ...
      if (raw.kind === 'tool_use') {
        return [createNormalizedMessage({
          ...
          toolId: typeof raw.toolId === 'string' ? raw.toolId : generateMessageId('omp'),
          sessionId,
          provider: PROVIDER,
        })];
      }
    ...
    if (eventType === 'tool_execution_start') {
      return [createNormalizedMessage({
        ...
        toolName: typeof raw.toolName === 'string' ? raw.toolName : 'Tool',
        toolInput: raw.args,
        toolId: typeof raw.toolCallId === 'string' ? raw.toolCallId : generateMessageId('omp'),
        sessionId,
        provider: PROVIDER,
      })];
    }
  ...
  async fetchHistory(
    ...
    // The route passes the app-facing session id, while Oh My Pi transcripts are
    ...
    const filePath = findOmpSessionFile(providerSessionId, this.sessionsRoot);
    ...
    for (const line of content.split(/\r?\n/)) {
        ...
        for (const [blockIndex, block] of blocks.entries()) {
          ...
          if (record.type === 'thinking' && typeof record.thinking === 'string' && record.thinking) {
            ...
            messages.push(createNormalizedMessage({
              ...
              toolId: typeof record.id === 'string' ? record.id : generateMessageId('omp'),
              timestamp,
              sessionId,
              provider: PROVIDER,
            }));
        ...
        if (message.stopReason === 'error' || message.stopReason === 'aborted') {
          messages.push(createNormalizedMessage({
            ...
            content: typeof message.errorMessage === 'string'
              ...
              : `Oh My Pi request ${message.stopReason}`,
            timestamp,
            sessionId,
            provider: PROVIDER,
          }));
      ...
      if (role === 'toolResult') {
        messages.push(createNormalizedMessage({
          ...
          toolId: typeof message.toolCallId === 'string' ? message.toolCallId : generateMessageId('omp'),
    ...
    // filled entirely with orphaned tool-result rows (which made Oh My Pi sessions
