...
export class QwenCodeSessionSynchronizer implements IProviderSessionSynchronizer {
  ...
  async synchronizeFile(filePath: string): Promise<string | null> {
    // The filename is normally the native id, but Qwen also persists the
    // canonical id in the transcript records. Return the id actually indexed
    // so watcher broadcasts target the same session row in either layout.
    return this.index(filePath);
  }

  private async index(filePath: string): Promise<string | null> {
    try {
