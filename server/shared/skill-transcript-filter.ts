...
/**
 * Filters one normalized event at a transcript boundary.
 *
 ...
 * by one live run or one history load and is intentionally not shared.
 */
...
/** Apply the live-event filter to a complete normalized history in order. */
export function filterSkillBodyEvents(messages: NormalizedMessage[]): NormalizedMessage[] {
  const skillReadToolIds = new Set<string>();
  const filtered: NormalizedMessage[] = [];

  for (const message of messages) {
    const next = filterSkillBodyEvent(message, skillReadToolIds);
    if (next) {
      filtered.push(next);
    }
  }
