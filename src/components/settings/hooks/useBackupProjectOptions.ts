...
export type BackupProjectOption = {
  fullPath: string;
  displayName: string;
};
...
/**
 * Loads the registered project list (skipping session sync) so the backup
 * exclusion picker can offer real projects by path identity.
 */
export function useBackupProjectOptions() {
  const [projects, setProjects] = useState<BackupProjectOption[]>([]);
  ...
  const refreshProjects = useCallback(async () => {
    ...
    try {
      ...
      const options = (Array.isArray(data) ? data : [])
        ...
        .filter((project): project is BackupProjectOption => project !== null)
