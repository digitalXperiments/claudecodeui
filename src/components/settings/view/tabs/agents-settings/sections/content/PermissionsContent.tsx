...
// Grok's real permission-rule syntax (`ToolPrefix(glob_pattern)`, verified
// against its own CLI docs and live-tested with `--allow`/`--deny`) - not
// the same prefix names as Cursor's `Shell(...)` convention above.
const COMMON_GROK_ALLOW_RULES = [
  'Bash(npm*)',
  'Bash(git*)',
  'Bash(ls*)',
  'Bash(cat*)',
...
function GrokPermissions({
  ...
  disallowedCommands,
  onDisallowedCommandsChange,
}: Omit<GrokPermissionsProps, 'agent'>) {
  const { t } = useTranslation('settings');
  const [newAllowedCommand, setNewAllowedCommand] = useState('');
  const [newDisallowedCommand, setNewDisallowedCommand] = useState('');
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-lg border border-orange-200 bg-orange-50 p-4 dark:border-orange-800 dark:bg-orange-900/20">
          <label className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-sm text-orange-700 dark:text-orange-300">
                {t('permissions.skipPermissions.grokDescription')}
              </div>
            </div>
          </label>
        </div>
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">{t('permissions.allowedCommands.grokDescription')}</p>

        <div className="flex flex-col gap-2 sm:flex-row">
          <Input
            value={newAllowedCommand}
            onChange={(event) => setNewAllowedCommand(event.target.value)}
            placeholder={t('permissions.allowedCommands.grokPlaceholder')}
            onKeyDown={(event) => {
              if (event.key === 'Enter') {
                event.preventDefault();
                handleAddAllowedCommand(newAllowedCommand);
        ...
        <div className="space-y-2">
          <p className="text-sm font-medium text-muted-foreground">
            {t('permissions.allowedCommands.grokQuickAdd')}
          </p>
          <div className="flex flex-wrap gap-2">
            {COMMON_GROK_ALLOW_RULES.map((command) => (
              <Button
                key={command}
                variant="outline"
                size="sm"
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">{t('permissions.blockedCommands.grokDescription')}</p>

        <div className="flex flex-col gap-2 sm:flex-row">
          <Input
            value={newDisallowedCommand}
            onChange={(event) => setNewDisallowedCommand(event.target.value)}
            placeholder={t('permissions.blockedCommands.grokPlaceholder')}
            onKeyDown={(event) => {
              if (event.key === 'Enter') {
                event.preventDefault();
                handleAddDisallowedCommand(newDisallowedCommand);
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {COMMON_GROK_DENY_RULES.map((command) => (
              <Button
                key={command}
                variant="outline"
                size="sm"
                onClick={() => handleAddDisallowedCommand(command)}
                disabled={disallowedCommands.includes(command)}
                className="h-8 text-xs"
              >
                {command}
              </Button>
      ...
      <div className="rounded-lg border border-purple-200 bg-purple-50 p-4 dark:border-purple-800 dark:bg-purple-900/20">
        <h4 className="mb-2 font-medium text-purple-900 dark:text-purple-100">
          {t('permissions.grokExamples.title')}
        </h4>
        <ul className="space-y-1 text-sm text-purple-800 dark:text-purple-200">
          <li><code className="rounded bg-purple-100 px-1 dark:bg-purple-800">"Bash(npm*)"</code> {t('permissions.grokExamples.bashNpm')}</li>
          <li><code className="rounded bg-purple-100 px-1 dark:bg-purple-800">"Bash(git*)"</code> {t('permissions.grokExamples.bashGit')}</li>
          <li><code className="rounded bg-purple-100 px-1 dark:bg-purple-800">"Bash(sudo*)"</code> {t('permissions.grokExamples.bashSudo')}</li>
          <li><code className="rounded bg-purple-100 px-1 dark:bg-purple-800">"Bash(rm*)"</code> {t('permissions.grokExamples.bashRm')}</li>
        </ul>
      </div>
    </div>
  );
