...
import { AlertTriangle, Check, Clock3, RefreshCw, Route, ShieldCheck, X } from 'lucide-react';
...
export default function ContinuityControl({
  ...
  return (
    <>
      ...
      >
        {recovery?.status === 'waiting' ? <Clock3 className="h-3.5 w-3.5" />
          : recovery?.status === 'running' ? <RefreshCw className="h-3.5 w-3.5 animate-spin" />
            : recovery?.status === 'needs_attention' || recovery?.status === 'failed' ? <AlertTriangle className="h-3.5 w-3.5" />
              : <ShieldCheck className="h-3.5 w-3.5" />}
        {statusLabel && <span aria-live="polite" className="max-w-20 truncate">{statusLabel}</span>}
      </button>
      ...
      {open && createPortal(
        ...
        >
          <div className="mb-3 flex items-start justify-between gap-3">
            <div>
              <div className="flex items-center gap-2 text-sm font-semibold"><ShieldCheck className="h-4 w-4 text-emerald-500" />Continuity</div>
              <p className="mt-1 text-[11px] leading-snug text-muted-foreground">CloudCLI can wait, resume, or hand the work to another provider when limits are reached.</p>
            </div>
            <button type="button" onClick={() => setOpen(false)} className="rounded p-1 text-muted-foreground hover:bg-muted" aria-label="Close"><X className="h-4 w-4" /></button>
          </div>
