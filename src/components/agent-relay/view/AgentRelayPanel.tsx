...
import AgentRelayView from './AgentRelayView';

type AgentRelayPanelProps = {
  isOpen: boolean;
  onClose: () => void;
  selectedProject: Project | null;
};

export default function AgentRelayPanel({
  isOpen,
  onClose,
  selectedProject,
}: AgentRelayPanelProps) {
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-hidden pb-safe-area-inset-bottom">
          <ErrorBoundary showDetails>
            <AgentRelayView selectedProject={selectedProject} isVisible={isOpen} />
          </ErrorBoundary>
        </div>
      </div>
    </div>
