...
const PROVIDERS = ['claude', 'grok', 'opencode', 'kilo', 'cline', 'codex', 'cursor', 'kimi', 'pi', 'omp', 'antigravity'];
const ICONS = ['🤖', '✨', '📬', '🧭', '🛠️', '📝', '🔎', '📚', '🎯', '⚡', '🌱', '🧠', '🗂️', '🧪', '🛰️', '🧹'];
const STEPS: Step[] = [
...
export default function BotArchitect({ mode, initialSection, projects, onSaved, onCancel }: BotArchitectProps): JSX.Element {
  ...
  useEffect(() => {
    try {
      ...
      if (saved && saved !== JSON.stringify(form)) {
  ...
  const loadTools = async (server: string) => {
    const open = !expandedServers[server];
    setExpandedServers((current) => ({ ...current, [server]: open }));
    if (!open || toolLists[server]) return;
    setToolLoading((current) => ({ ...current, [server]: true }));
    try {
      const response = await authenticatedFetch(`/api/providers/mcp/catalog/${encodeURIComponent(server)}/tools`);
      const body = await response.json() as { tools?: ToolInfo[] };
      setToolLists((current) => ({ ...current, [server]: body.tools ?? [] }));
      setForm((current) => {
        const existing = getServerTools(current.tool_policy ?? {}, server);
        const next = Object.fromEntries((body.tools ?? []).map((tool) => [tool.name, existing[tool.name] ?? 'allow'])) as Record<string, ToolPolicyDecision>;
        return { ...current, tool_policy: { ...current.tool_policy, [server]: next } };
      });
    } catch {
      setNotice(`Could not load tools for ${server}.`);
    } finally {
  ...
  const save = async () => {
    ...
    if (!isValidCron(form.schedule_cron)) { setError('Trigger cron must contain exactly five fields.'); setStep(5); return; }
    setSaving(true);
    try {
      const payload: CreateMcSectionInput = { ...form, title: form.title.trim(), schedule_cron: form.schedule_cron?.trim() || null };
  ...
  const runFirstTick = async () => {
    if (!initialSection?.section_id) return;
    try {
      const result = await missionControlApi.runSection(initialSection.section_id);
      setRunResult({ created: result.created, skipped: result.skipped });
    } catch (caught) {
      setNotice(caught instanceof Error ? caught.message : 'The first tick could not start.');
    }
  };
  ...
  const renderAgent = () => (
    <StepPanel eyebrow="02 · Agent" title="Choose the mind and the safety boundary" description="Provider and model are loaded from the same model registry used by Mission Control.">
      <div className="grid gap-4 md:grid-cols-2"><div><FieldLabel>Provider</FieldLabel><select className="field" value={form.provider ?? 'claude'} onChange={(event) => updateForm({ provider: event.target.value, model: null })}>{PROVIDERS.map((provider) => <option key={provider} value={provider}>{provider}</option>)}</select></div><div><FieldLabel detail={modelsLoading ? 'loading…' : undefined}>Model</FieldLabel><select className="field" value={form.model ?? ''} onChange={(event) => updateForm({ model: event.target.value || null })}><option value="">Provider default</option>{models.map((modelOption) => <option key={modelOption.value} value={modelOption.value}>{modelOption.label}</option>)}</select></div></div>
  ...
  const renderTriggers = () => <StepPanel eyebrow="05 · Triggers" title="When should this bot tick?" description="A tick is a fresh run. Keep the schedule explicit and easy to inspect.">
    <div><FieldLabel>Schedule</FieldLabel><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{CRON_PRESETS.map((preset) => <button key={preset.value} type="button" className={`choice ${form.schedule_cron === preset.value ? 'choice-active' : ''}`} onClick={() => updateForm({ schedule_cron: preset.value })}><Clock3 className="h-4 w-4" />{preset.label}<span>{preset.value}</span></button>)}</div></div>
    <div><FieldLabel detail="five fields">Raw cron</FieldLabel><input className={`field font-mono ${form.schedule_cron && !isValidCron(form.schedule_cron) ? 'border-red-500' : ''}`} value={form.schedule_cron ?? ''} onChange={(event) => updateForm({ schedule_cron: event.target.value })} placeholder="*/30 * * * *" /><p className="mt-1.5 text-[11px] text-muted-foreground">{cronSummary(form.schedule_cron)}</p></div>
  ...
  const renderOutputs = () => <StepPanel eyebrow="06 · Outputs & actions" title="Decide what approval means" description="Autonomy maps directly to the existing Mission Control section contract.">
    ...
    {form.create_kanban_task ? <div className="grid gap-3 sm:grid-cols-2"><div><FieldLabel>Assignee provider</FieldLabel><select className="field" value={form.kanban_assignee_provider ?? ''} onChange={(event) => updateForm({ kanban_assignee_provider: event.target.value || null })}><option value="">Default</option>{PROVIDERS.map((provider) => <option key={provider} value={provider}>{provider}</option>)}</select></div><div><FieldLabel>Review provider</FieldLabel><select className="field" value={form.kanban_review_provider ?? ''} onChange={(event) => updateForm({ kanban_review_provider: event.target.value || null })}><option value="">Default</option>{PROVIDERS.map((provider) => <option key={provider} value={provider}>{provider}</option>)}</select></div></div> : null}
  ...
  const renderReview = () => <StepPanel eyebrow="08 · Review" title="Ready to create this bot?" description="New bots start paused by default in this flow. Create it, inspect the first result, then enable it when it earns trust.">
    ...
    {mode === 'edit' && initialSection?.section_id ? <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border/60 bg-muted/20 p-3"><button type="button" className="button" onClick={() => void runFirstTick()}><Sparkles className="h-4 w-4" />Run first tick now</button>{runResult ? <span className="text-xs text-muted-foreground">{runResult.created} created{runResult.skipped ? ` · ${runResult.skipped} skipped` : ''}</span> : null}</div> : null}
  </StepPanel>;
