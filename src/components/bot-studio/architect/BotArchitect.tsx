...
export default function BotArchitect({ mode, initialSection, projects, onSaved, onCancel }: BotArchitectProps): JSX.Element {
  ...
  const loadTools = async (server: string) => {
    ...
    try {
      ...
      setToolLists((current) => ({ ...current, [server]: serverTools }));
      setForm((current) => {
        const existing = getServerTools(current.tool_policy ?? {}, server);
        const next = Object.fromEntries(serverTools.map((tool) => [tool.name, existing[tool.name] ?? defaultToolDecision(tool.name, Boolean(current.read_only_preset))])) as Record<string, ToolPolicyDecision>;
        return { ...current, tool_policy: { ...current.tool_policy, [server]: next } };
      });
    } catch (caught) {
      setToolErrors((current) => ({ ...current, [server]: caught instanceof Error ? caught.message : `Could not load tools for ${server}.` }));
    } finally {
