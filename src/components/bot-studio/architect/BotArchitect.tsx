...
function sectionInput(initial?: BotArchitectProps['initialSection']): CreateMcSectionInput {
  return {
    ...
    work_project_id: initial?.work_project_id ?? null,
    ...
    <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-sm">
      <div className="border-b border-border/60 bg-gradient-to-br from-primary/[0.10] via-card to-violet-500/[0.07] p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          ...
          <span className="rounded-full border border-border/70 bg-background/70 px-2 py-1 text-[10px] text-muted-foreground">{form.work_project_id ? 'Work project assigned' : 'Work project: match'}</span>
        </div>
      </div>
...
export default function BotArchitect({ mode, initialSection, projects, onSaved, onCancel }: BotArchitectProps): JSX.Element {
  ...
  const renderPurpose = () => (
    <StepPanel eyebrow="01 · Purpose" title="What should this bot do?" description="A clear outcome gives the Architect enough signal to draft the rest. You can refine every field later.">
      ...
      <div className="grid gap-4 md:grid-cols-[1fr_1.4fr]">
        <div><FieldLabel>Run scope</FieldLabel><div className="grid grid-cols-2 gap-2">{(['global', 'project'] as const).map((scope) => <button key={scope} type="button" className={`choice ${form.scope === scope ? 'choice-active' : ''}`} onClick={() => updateForm({ scope, project_id: scope === 'global' ? null : form.project_id })}>{scope === 'global' ? 'Global' : 'Project'}<span>{scope === 'global' ? 'User home' : 'Run in one project'}</span></button>)}</div></div>
        {form.scope === 'project' ? <div><FieldLabel>Bot run project</FieldLabel><select className="field" value={form.project_id ?? ''} onChange={(event) => updateForm({ project_id: event.target.value || null })}><option value="">Select a project…</option>{projects.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name}</option>)}</select></div> : <div className="rounded-xl border border-border/60 bg-muted/25 p-3 text-xs text-muted-foreground">Global bots run from the user home. Choose Project when this bot should read local files from a registered project.</div>}
      </div>
      <div className="rounded-xl border border-primary/20 bg-primary/[0.04] p-4"><FieldLabel detail="used only for the Work this handoff">Work this project</FieldLabel><select className="field" value={form.work_project_id ?? ''} onChange={(event) => updateForm({ work_project_id: event.target.value || null })}><option value="">Match or choose per inbox item</option>{projects.map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name}{candidate.id === form.project_id ? ' · same as bot run project' : ''}</option>)}</select><p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">When assigned, Work this always opens the item in this project. It is independent from the project where the bot itself runs.</p></div>
  ...
  const renderReview = () => <StepPanel eyebrow="08 · Review" title="Ready to create this bot?" description="New bots start paused by default in this flow. Create it, inspect the first result, then enable it when it earns trust.">
    <div className="divide-y divide-border/60 overflow-hidden rounded-xl border border-border/70">{[['Name', form.title || 'Untitled bot'], ['Purpose', form.produce_prompt || 'Not set'], ['Bot run project', form.scope === 'project' ? project?.name || 'Project not selected' : 'User home (global)'], ['Work this project', form.work_project_id ? projects.find((candidate) => candidate.id === form.work_project_id)?.name || 'Project not found' : 'Choose per item'], ['Agent', `${form.provider || 'claude'}${form.model ? ` · ${form.model}` : ''}`], ['Autonomy', autonomy === 'dry_run' ? 'Dry run' : autonomy === 'act' ? 'Act' : 'Propose'], ['Trigger', cronSummary(form.schedule_cron)], ['Tools', attachedServers.length ? attachedServers.join(', ') : 'No MCP servers attached'], ['Actions', `${form.actions?.length ?? 0} action${form.actions?.length === 1 ? '' : 's'}`]].map(([label, value]) => <div key={label} className="grid gap-1 px-4 py-3 sm:grid-cols-[150px_1fr]"><span className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">{label}</span><span className="truncate text-xs text-foreground">{value}</span></div>)}</div>
