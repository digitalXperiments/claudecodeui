  ...
  const loadTools = async (server: string) => {
    ...
    try {
      ...
      setForm((current) => {
        ...
        const next = Object.fromEntries((body.tools ?? []).map((tool) => [tool.name, existing[tool.name] ?? defaultToolDecision(tool.name, Boolean(current.read_only_preset))])) as Record<string, ToolPolicyDecision>;
