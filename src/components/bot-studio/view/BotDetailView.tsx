...
export default function BotDetailView({ bot, projectName, workProjectName, items, runs, onUpdate, onRun, onCancelRun, onDelete, onDuplicate, onEdit, onSelectItem, selectedTab, onTabChange, onSelectRun }: BotDetailViewProps) {
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col bg-muted/10">
      <header className="shrink-0 border-b border-border/70 bg-background px-4 py-4 sm:px-6">
        <div className="flex flex-wrap items-start gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-primary/10 text-lg text-primary"><BotIcon icon={bot.icon} size={24} /></div>
          <div className="min-w-0 flex-[1_1_16rem]">
            <div className="flex flex-wrap items-center gap-1.5">
              <h2 className="truncate text-base font-semibold">{bot.title}</h2>
              <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] font-medium capitalize text-muted-foreground">{bot.scope}</span>
              {bot.project_id ? <span className="max-w-48 truncate rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground" title={bot.project_id}>Run · {projectName || bot.project_id}</span> : null}
              {bot.work_project_id ? <span className="max-w-48 truncate rounded-full bg-primary/10 px-2 py-0.5 text-[10px] text-primary" title={bot.work_project_id}>Work · {workProjectName || bot.work_project_id}</span> : null}
              <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">{bot.provider}{bot.model ? ` · ${bot.model}` : ''}</span>
              <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">{bot.schedule_cron ? `Trigger · ${bot.schedule_cron}` : 'Manual'}</span>
            </div>
            <p className="mt-1 line-clamp-1 text-xs text-muted-foreground">{bot.purpose}</p>
            <p className="mt-1 whitespace-nowrap text-[10px] text-muted-foreground">{bot.last_run_at ? `Last tick ${formatAge(bot.last_run_at)}` : 'No ticks yet'}</p>
          </div>
          <div className="flex shrink-0 basis-auto items-center gap-2 max-[720px]:basis-full max-[720px]:justify-end">
            ...
            <div className="relative">
              ...
              {menuOpen ? <div className="absolute right-0 top-10 z-10 w-44 rounded-xl border border-border bg-popover p-1 shadow-lg"><button type="button" onClick={() => void duplicate()} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Copy className="h-3.5 w-3.5" />Duplicate bot</button><button type="button" onClick={() => void deleteBot()} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs text-destructive hover:bg-destructive/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Trash2 className="h-3.5 w-3.5" />Delete bot</button></div> : null}
            </div>
          </div>
        </div>
