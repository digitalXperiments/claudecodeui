...
export default function ToolsTab({ bot, onSave }: { bot: Bot; onSave: (patch: Partial<CreateMcSectionInput>) => Promise<void> }) {
  ...
  useEffect(() => { let cancelled = false; void Promise.all(attached.map(async (server) => [server, await botStudioApi.listMcpTools(server)] as const)).then((entries) => { if (!cancelled) setTools(Object.fromEntries(entries)); }).catch(() => undefined); return () => { cancelled = true; }; }, [attached]);
