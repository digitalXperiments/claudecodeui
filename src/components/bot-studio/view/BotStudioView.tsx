...
export default function BotStudioView({ projects, isMobile, onMenuClick, onBackToChat, onWorkThis }: { projects: Project[]; isMobile: boolean; onMenuClick?: () => void; onBackToChat: () => void; onWorkThis?: (request: WorkThisSessionRequest) => void }) {
  ...
  const activeView: BotStudioViewKey = route.page === 'new' || route.page === 'import' ? 'inbox' : route.page;
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background text-foreground">
    ...
    <BotStudioHeader activeView={activeView} counts={{ inbox: inboxCount, bots: data.bots.length, templates: BOT_TEMPLATES.length, activity: activityCount }} search={search} onSearchChange={setSearch} onViewChange={(nextView) => openView(nextView)} onBackToChat={onBackToChat} onNewBot={() => openArchitect()} onPauseAll={() => void pauseOrResumeAll()} pauseAllLabel={allPaused ? 'Resume all' : 'Pause all'} onToggleRoster={layout.toggleRoster} onToggleContext={layout.toggleContext} rosterOpen={layout.rosterOpen} contextOpen={layout.contextOpen} />
