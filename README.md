...
- **[Antigravity](https://antigravity.google/)** (Google) — driven through Google's official ACP agent, the same approach [T3 Code](https://github.com/pingdotgg/t3code) takes. **Off by default:** enable it in Settings → Agents → Antigravity, run **Install** (CloudCLI downloads the official archive for your host, verifies its SHA-256 *and* byte size against a pin that ships in the repo, and extracts the ACP executable plus `localharness_external` into `~/.cloudcli/antigravity/<version>/`), then **Sign in with Google**. Auth is personal Google only (ACP `authenticate` with `oauth-personal`) — there is no Gemini API-key fallback, so an unavailable runtime can never silently become metered API billing. Sign-in on a remote CloudCLI is completed by pasting the `http://127.0.0.1…` redirect URL back into Settings. Permission modes map onto Antigravity's own session modes (`default` / `auto_edit` / `yolo`); it has no read-only plan agent and no rewind, and CloudCLI does not pretend otherwise. Supported hosts are macOS arm64, Linux x64/arm64 and Windows x64/arm64 — Intel Macs are reported as unsupported rather than hidden. Models come from the agent's own ACP `model` config option, never a hardcoded Gemini list.

### Cross-agent capabilities (work across every provider)

- **Model Capability Registry** — every model across installed providers is discovered, scored from public coding benchmarks (`server/modules/swarm/model-benchmarks.snapshot.json`), and cached in the `model_capabilities` table. Manage it under Settings → Model registry. Agent Swarm is retired; delegated work uses Agent Relay from the left sidebar rail (`docs/agent-relay.md`).
